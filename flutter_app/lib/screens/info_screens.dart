...
const _termsSections = [
  PolicySection('제1조 (목적)',
      '이 약관은 트리포드(Tripod) 팀(이하 "팀")이 제공하는 반값여행 정보 제공·여행 기록·환급 증빙 보조 서비스 "하프트립"(이하 "서비스")의 이용 조건과 팀·이용자의 권리, 의무 및 책임사항을 정합니다.'),
  ...
  PolicySection('제4조 (계정)',
      '1. 서비스는 카카오·구글 계정 연동으로 가입할 수 있으며, 만 14세 미만은 가입할 수 없습니다.\n'
  ...
  PolicySection('제6조 (게시물)',
      '1. 커뮤니티 게시물의 저작권은 작성자에게 있습니다.\n'
          ...
          '3. 이용자는 부적절한 게시물·댓글을 앱 내 신고 기능으로 신고할 수 있으며, 팀은 신고된 게시물을 검토하여 필요한 조치를 합니다.\n'
          '4. 제5조를 위반한 게시물은 사전 통지 없이 삭제될 수 있습니다.'),
  ...
  PolicySection('제10조 (콘텐츠 출처 고지)',
      '1. 서비스 내 관광지·장소·축제 등 일부 관광 정보는 한국관광공사가 제공하는 오픈API를 통해 제공받으며, 해당 정보가 표시되는 화면에는 "출처: ⓒ한국관광공사"를 표기합니다.\n'
          '2. 지도 및 일부 장소 정보는 Google, 카카오 등 외부 서비스를 통해 제공될 수 있으며, 각 정보의 권리는 해당 제공자에게 있습니다.'),
];

const _privacySections = [
  PolicySection('1. 총칙',
      '"하프트립"을 운영하는 트리포드(Tripod) 팀(이하 "팀")은 개인정보 보호법 제30조에 따라 이용자의 개인정보를 보호하고 관련 고충을 신속하게 처리하기 위해 다음과 같이 개인정보 처리방침을 수립·공개합니다.'),
  PolicySection('2. 처리하는 개인정보의 항목·목적·보유기간',
      '① 회원 관리 — 소셜 로그인 식별자(카카오·구글 회원번호), 이메일, 거주지(시·군·구), 닉네임·프로필 설정 / 회원 탈퇴 시까지\n'
          '② 환급 증빙 지원 — 성명, 전화번호 (숙박확인서 작성 시 입력받아 해당 여행의 숙박확인서 기록에만 저장) / 회원 탈퇴 또는 삭제 요청 시까지\n'
          '③ 환급 증빙 보조 — 인증사진·영수증·숙박확인서 이미지 및 사진에 포함된 촬영 시각·위치(GPS) 정보, 전자서명 / 회원 탈퇴 또는 삭제 요청 시까지\n'
          '④ 알림 제공 — 푸시 토큰, 알림 설정 / 회원 탈퇴 시까지\n'
          '⑤ 서비스 이용 기록(접속 로그) / 3개월\n'
          '⑥ 커뮤니티 운영 — 게시글·댓글 내용, 첨부한 코스 정보, 좋아요·저장 기록 / 이용자가 삭제할 때까지. 회원 탈퇴 시 게시글·댓글은 작성자 정보가 비식별화("탈퇴한 사용자")된 상태로 남으며, 탈퇴 전 직접 삭제할 수 있습니다.\n'
  ...
  PolicySection('8. 안전성 확보 조치',
      '팀은 개인정보의 안전한 처리를 위해 통신 구간 암호화(HTTPS), 접근 권한 최소화, 비밀번호의 일방향 암호화 저장, 실명·연락처의 수집 시점·범위 최소화(숙박확인서 작성 시에만, 해당 여행 기록에 한해 수집) 등의 조치를 시행합니다.'),
  PolicySection('9. 개인정보 보호책임자',
      '개인정보 보호책임자: 조하민\n'
          '문의: jhm0350@gmail.com\n'
