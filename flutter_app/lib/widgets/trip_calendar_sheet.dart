...
/// 여행 일정(기간) 선택 — 디자인 시스템 톤의 커스텀 범위 캘린더 바텀시트.
...
class _TripCalendarSheetState extends State<_TripCalendarSheet> {
  ...
  void initState() {
    ...
    _start = widget.initial == null
        ? null
        : DateUtils.dateOnly(widget.initial!.start);
    _end =
        widget.initial == null ? null : DateUtils.dateOnly(widget.initial!.end);
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text('여행 일정', style: Theme.of(context).textTheme.titleLarge),
...
class _NavButton extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return IconButton(
      ...
      icon: Icon(icon,
          size: 26, color: onTap == null ? AppColors.ink4 : AppColors.ink7),
    );
  }
}
            ...
            Row(
              children: [
                Expanded(
                  child: Container(
                    height: 38,
                    color: isStart ? Colors.transparent : AppColors.p50,
                  ),
                ),
                Expanded(
                  child: Container(
                    height: 38,
                    color: isEnd ? Colors.transparent : AppColors.p50,
                  ),
                ),
              ],
            ),
