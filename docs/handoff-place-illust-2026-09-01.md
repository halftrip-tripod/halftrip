...
| 파일명 | 소재 |
| --- | --- |
| `default` | **여행 캐리어**(모자·카메라·지도가 담긴) — 2026-09-03 준서가 이정표·캐리어 두 안을 만들었고 캐리어로 확정 |
...
- 확장자 `.png`, **투명 배경**
- 규희가 **긴 변 512px**로 줄여서 넣는다 (1254px 원본 32장이 53MB였고, 512px로 11MB)
...
- 반영 이력: 2026-09-03 31종 전부 들어옴 → `kAvailablePlaceIllusts`에 키 등록(`feat/place-illust-enable`)
